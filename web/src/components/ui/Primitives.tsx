...
export function Button({ variant = "primary", size = "md", children, icon, onClick, disabled, style, type = "button" }: ButtonProps) {
...
type Tone = "positive" | "neutral" | "caution" | "danger" | "accent" | "mono";
export function Badge({ tone = "neutral", children, dot = false, pulse = false }: { tone?: Tone; children?: ReactNode; dot?: boolean; pulse?: boolean }) {
  ...
  return (
    <span style={{
      ...
      ...(tone === "mono" ? { fontFamily: "var(--font-mono)" } : {}),
    }}>
      {dot && <span style={{ width: 6, height: 6, borderRadius: 999, background: t.fg, animation: pulse ? "echo-pulse 1.6s var(--ease-in-out) infinite" : "none" }} />}
      {children}
    </span>
  );
}
...
export function EchoMark({ size = 24, accent = true }: { size?: number; accent?: boolean }) {
  const c = accent ? "#d4ff5c" : "currentColor";
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      <circle cx="11" cy="16" r="2.5" fill={c} />
      <path d="M16 10.5 A 6 6 0 0 1 16 21.5" stroke={c} strokeWidth="2" strokeLinecap="round" />
      <path d="M20.5 7.5 A 10.5 10.5 0 0 1 20.5 24.5" stroke={c} strokeWidth="2" strokeLinecap="round" opacity="0.55" />
      <path d="M25 4.5 A 15 15 0 0 1 25 27.5" stroke={c} strokeWidth="2" strokeLinecap="round" opacity="0.25" />
    </svg>
  );
}
