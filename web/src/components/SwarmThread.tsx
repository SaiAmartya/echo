...
type Archetype = "enthusiast" | "practitioner" | "curious" | "lurker" | "pedant" | "skeptic";
type Audience = "target" | "public";

type Agent = { id: string; name: string; handle: string; archetype: Archetype; audience: Audience };
...
type Post = { id: string; round: number; parent: string; agent: string; sentiment: number; text: string };

const THREAD_SCRIPT: Post[] = [
  ...
  { id: "p14", round: 4, parent: "p2",   agent: "a7",  sentiment: -0.05, text: "fwiw the absolutist phrasing is the part drawing fire, not the idea." },
...
function getAgent(id: string) { return SEED_AGENTS.find((a) => a.id === id); }
function getInitials(name: string) { return name.split(" ").map((s) => s[0]).slice(0, 2).join("").toUpperCase(); }
function toneColor(s: number) { return s > 0.15 ? "#7dd49a" : s < -0.15 ? "#f06c5a" : "#b8b8c0"; }
...
export function SwarmThread({ currentRound = 5, seedDraft = "", running = false }: { currentRound?: number; seedDraft?: string; running?: boolean }) {
  const posts = useMemo(() => THREAD_SCRIPT.filter((p) => p.round <= currentRound), [currentRound]);

  const edges = useMemo(() => posts.map((p) => {
    const ag = getAgent(p.agent);
    if (!ag) return null;
    const me = agentPoint(ag.id, ag.archetype);
    if (p.parent === "seed") return { from: me, to: { x: 0, y: 0, color: "#d4ff5c" }, sentiment: p.sentiment };
  ...
  }).filter(Boolean) as { from: { x: number; y: number }; to: { x: number; y: number }; sentiment: number }[], [posts]);

  const dogpileIds = useMemo(() => {
    const counts: Record<string, number> = {};
    posts.forEach((p) => { counts[p.parent] = (counts[p.parent] || 0) + 1; });
    return new Set(Object.entries(counts).filter(([k, v]) => k !== "seed" && v >= 2).map(([k]) => k));
  }, [posts]);
...
function ThreadColumn({ seedDraft, posts }: { seedDraft: string; posts: Post[] }) {
  const memo: Record<string, number> = {};
  const depthOf = (p: Post): number => {
  ...
  const lastRound = enriched.length ? Math.max(...enriched.map((p) => p.round)) : 0;

  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 10, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        <span style={{ fontSize: 14, color: "var(--fg-1)", fontWeight: 500 }}>The thread</span>
        <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-3)" }}>{posts.length} replies</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8, paddingRight: 4 }}>
        <div style={{ background: "var(--bg-deep)", border: "1px solid var(--border)", borderLeft: "2px solid var(--accent-200)", borderRadius: 10, padding: 12 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 500, color: "var(--fg-1)" }}>your post</span>
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-3)" }}>@you</span>
          </div>
        ...
        {enriched.map((p) => {
          ...
          return (
            <div key={p.id} style={{ marginLeft: p.depth * 18, borderLeft: p.depth > 0 ? "1px solid var(--border)" : "none", paddingLeft: p.depth > 0 ? 10 : 0 }}>
              <div style={{ background: isLatest ? "var(--surface-2)" : "transparent", border: `1px solid ${isLatest ? "var(--border-strong)" : "var(--border)"}`, borderRadius: 10, padding: "10px 12px", display: "flex", gap: 10, transition: "background 200ms" }}>
                <div style={{ width: 24, height: 24, borderRadius: 999, flexShrink: 0, background: "var(--surface-3)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--fg-2)" }}>{getInitials(ag.name)}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                    ...
                    <span style={{ marginLeft: "auto", width: 6, height: 6, borderRadius: 999, background: toneColor(p.sentiment) }} />
                  </div>
...
function SwarmMap({ posts, edges, dogpileIds, running }: { posts: Post[]; edges: { from: { x: number; y: number }; to: { x: number; y: number }; sentiment: number }[]; dogpileIds: Set<string>; running: boolean }) {
  const VW = 360, VH = 280;
  ...
  const dogpilePositions = posts
    ...
    .filter((x): x is { x: number; y: number; color: string } => x !== null);

  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 12, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        <span style={{ fontSize: 14, color: "var(--fg-1)", fontWeight: 500 }}>The room</span>
        <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--fg-3)" }}>{activeAgents.size} / 200 engaged</span>
      </div>
      <div style={{ flex: 1, position: "relative", background: "radial-gradient(circle at center, rgba(212,255,92,0.04) 0%, transparent 65%), var(--bg-deep)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", minHeight: 220 }}>
        <svg viewBox={`${-VW / 2} ${-VH / 2} ${VW} ${VH}`} width="100%" height="100%" style={{ position: "absolute", inset: 0 }}>
          {(Object.entries(CLUSTER_CENTERS) as [Archetype, { x: number; y: number; color: string }][]).map(([k, c]) => (
            <circle key={k} cx={c.x} cy={c.y} r="48" fill={c.color} opacity="0.05" />
          ))}
          {edges.map((e, i) => (
            <line key={i} x1={e.from.x} y1={e.from.y} x2={e.to.x} y2={e.to.y} stroke={toneColor(e.sentiment)} strokeWidth="0.6" opacity="0.45" />
          ))}
          {SEED_AGENTS.map((a) => {
            const p = agentPoint(a.id, a.archetype);
            const active = activeAgents.has(a.id);
            return (
              <circle key={a.id} cx={p.x} cy={p.y} r={active ? 3 : 2}
                fill={active ? p.color : "#2e2e34"} opacity={active ? 0.95 : 0.5}
                style={{ animationName: active && running ? "echo-pulse" : "none", animationDuration: "1.6s", animationTimingFunction: "cubic-bezier(0.65, 0, 0.35, 1)", animationIterationCount: "infinite" }} />
            );
          })}
          ...
          <circle cx="0" cy="0" r="2" fill="var(--accent-200)" style={{ animationName: "echo-pulse", animationDuration: "1.6s", animationTimingFunction: "cubic-bezier(0.65, 0, 0.35, 1)", animationIterationCount: "infinite" }} />
          {dogpilePositions.map((p, i) => (
            <circle key={`dp${i}`} cx={p.x} cy={p.y} r="9" fill="none" stroke="var(--accent-200)" strokeWidth="1" opacity="0.7"
              style={{ animationName: "echo-ripple", animationDuration: "2.4s", animationTimingFunction: "cubic-bezier(0.2, 0.8, 0.2, 1)", animationIterationCount: "infinite", animationDelay: `${i * 0.3}s`, transformOrigin: `${p.x}px ${p.y}px` }} />
          ))}
        </svg>
      </div>
    </div>
