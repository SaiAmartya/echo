// Primitives: Button, Badge, Avatar, Icon (Lucide-style inline SVG)

const Button = ({ variant = 'primary', size = 'md', children, icon, onClick, disabled, style }) => {
  const base = {
    fontFamily: 'var(--font-sans)', fontWeight: 500, lineHeight: 1,
    borderRadius: 6, border: '1px solid transparent', cursor: disabled ? 'not-allowed' : 'pointer',
...
const Badge = ({ tone = 'neutral', children, dot = false, pulse = false }) => {
  const tones = {
  ...
  const t = tones[tone];
  const monoFont = tone === 'mono' ? { fontFamily: 'var(--font-mono)' } : {};
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap',
...
const Avatar = ({ initials, size = 32 }) => (
  <div style={{
    ...
    fontFamily: 'var(--font-mono)', fontSize: size * 0.35, color: 'var(--fg-2)',
    flexShrink: 0,
  }}>{initials}</div>
);
...
const Icon = ({ name, size = 16, style }) => {
  ...
  const paths = {
    ...
    users: <g><circle cx="9" cy="8" r="3" /><path d="M3 19c0-3 3-5 6-5s6 2 6 5" /><circle cx="17" cy="9" r="2" /><path d="M21 18c0-2-2-3-4-3" /></g>,
    replies: <g><path d="M7 9h10M7 13h6" /><path d="M21 12a9 9 0 1 1-3.5-7.1L21 4v5h-5" /></g>,
    trendingUp: <g><polyline points="3,17 9,11 13,15 21,7" /><polyline points="14,7 21,7 21,14" /></g>,
    trendingDown: <g><polyline points="3,7 9,13 13,9 21,17" /><polyline points="14,17 21,17 21,10" /></g>,
    alert: <g><path d="M12 3 22 20H2Z" /><path d="M12 10v4" /><circle cx="12" cy="17" r="0.5" fill="currentColor" /></g>,
    check: <g><circle cx="12" cy="12" r="9" /><polyline points="8,12 11,15 16,9" /></g>,
    x: <g><circle cx="12" cy="12" r="9" /><line x1="9" y1="9" x2="15" y2="15" /><line x1="15" y1="9" x2="9" y2="15" /></g>,
    search: <g><circle cx="11" cy="11" r="7" /><path d="m20 20-3-3" /></g>,
    plus: <path d="M12 5v14M5 12h14" />,
    home: <g><path d="M3 11 12 4l9 7v9a1 1 0 0 1-1 1h-4v-7H10v7H4a1 1 0 0 1-1-1z" /></g>,
    ...
    sparkles: <g><path d="M12 3v4M12 17v4M3 12h4M17 12h4" /></g>,
    refresh: <g><path d="M21 12a9 9 0 1 1-3-6.7" /><polyline points="21,4 21,9 16,9" /></g>,
  };
  return <svg {...props}>{paths[name] || null}</svg>;
};

const EchoMark = ({ size = 24, accent = true }) => (
  <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
    <circle cx="11" cy="16" r="2.5" fill={accent ? '#d4ff5c' : 'currentColor'} />
    <path d="M16 10.5 A 6 6 0 0 1 16 21.5" stroke={accent ? '#d4ff5c' : 'currentColor'} strokeWidth="2" strokeLinecap="round" />
    <path d="M20.5 7.5 A 10.5 10.5 0 0 1 20.5 24.5" stroke={accent ? '#d4ff5c' : 'currentColor'} strokeWidth="2" strokeLinecap="round" opacity="0.55" />
    <path d="M25 4.5 A 15 15 0 0 1 25 27.5" stroke={accent ? '#d4ff5c' : 'currentColor'} strokeWidth="2" strokeLinecap="round" opacity="0.25" />
  </svg>
);

Object.assign(window, { Button, Badge, Avatar, Icon, EchoMark });
