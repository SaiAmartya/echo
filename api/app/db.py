...
def insert_simulation(
    ...
    web_grounding (bool): when true, /simulate/stream will run an extra Gemini
    call with google_search grounding before the rounds. Persisted as INTEGER
    0/1. Default False.

    persona_count (int | None): Z1 / v7 persona pool size; NULL on v6 sims.
    """
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO simulations "
            "(id, user_id, audience_id, draft, rounds, mode, web_grounding, persona_count) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                sim_id,
                user_id,
